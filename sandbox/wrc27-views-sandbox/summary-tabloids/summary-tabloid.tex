...
\setstretch{1.05}
Figures show the radio bands under consideration in various WRC-27 ``Agenda Items'' along with bands allocated to and/or afforded some form of protection for scientific use in the International Telecommunications Union (ITU) Radio Regulations (RR). The top row in each figure notes the band(s) under consideration in the Agenda Item (grey). Rows beneath denote bands within the same spectral region that are allocated to (or afforded some level of acknowledgement/protection for) scientific observations, either for radio astronomy (the Radio Astronomy Service, RAS, purples/pinks) or Earth observation (the Earth Exploration Satellite Service, EESS, passive or active, greens). Any bands afforded ``all emissions prohibited'' protection under RR Footnote 5.340 will be indicated (in orange) in the bottommost row. RAS and/or EESS bands shown in strong colors are primary allocations (annotated with a ``Pri.\@'' in the legend above), those shown with paler colors are secondary allocations (annotated with a ``Sec\@.'' in the legend), and bands shown in the palest colors are afforded protections (or otherwise identified) in a specific footnote of the, such as RR~5.149 (annotated with ``Fn.'' in the legend).  Arrows are placed within (or beside for very narrow bands) bands under consideration in an Agenda Item denote cases where transmissions are space-to-Earth, Earth-to-space, and/or space-to-space. For clarity, particularly narrow bands that would otherwise not be visible on the page are shown with exaggerated bandwidth.}}
...
\includegraphics[width=\linewidth]{draft-report-cover.png}\par
